...
Renderer::Renderer (std::vector<SceneNode*> &scenes, std::vector<std::vector<int> > &edges) {
	this->num_nodes = scenes.size () ; // number of meshes
	this->num_edges = edges.size () ; // number of edges
...
void Renderer::populateMap (std::vector<SceneNode*> &scenes) {
	...
	for (auto &scene:scenes) {
		this->sceneMap[id++] = scene ; // post increment, so each scene is mapped to its nodeId
	}
}
